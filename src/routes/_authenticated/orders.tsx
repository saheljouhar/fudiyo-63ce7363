...
import {
  ...
  CreditCard, Printer, Plus as PlusIcon, MapPin, Armchair,
} from "lucide-react";
...
interface SavedCart { id: string; label: string; cart: CartItem[]; orderType: OrderType; at: string }
...
function OrdersPage() {
  ...
  const saveCart = () => {
    ...
    setSaved((s) => [...s, { id, label: `Cart - ${at}`, cart, orderType, at }]);
    setCart([]);
  ...
  return (
    <div className="-mt-4 fixed left-0 right-0 bottom-0" style={{ top: 60 }}>
      <TopNav toggleSidebar={toggle} search={search} setSearch={setSearch} tableNum={tableNum} />
      <div className="flex bg-[#F9FAFB]" style={{ height: "calc(100vh - 60px - 56px)" }}>
        {/* LEFT */}
        {!topBarMode && (
        ...
        {/* RIGHT - Order summary */}
        <aside className="w-[300px] shrink-0 bg-white border-l border-[#E5E7EB] flex flex-col">
          {/* Header */}
            ...
            <div className="flex items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="text-[15px] font-bold leading-tight">Order Summary</div>
          ...
          {/* Delivery */}
          {orderType === "delivery" && post.kind === "none" && (
          ...
          <div className="flex-1 overflow-y-auto">
            {post.kind !== "none" ? (
              ...
              <div className="text-center py-16 px-4">
                <ShoppingCart className="size-14 mx-auto mb-3 text-[#CBD5E1]" />
                <div className="text-[14px] text-[#6B7280] font-semibold">Your cart is empty</div>
                <div className="text-[12px] text-[#9CA3AF] mt-1">Add items from the menu to start</div>
              </div>
            ) : (
              <div className="p-3 space-y-2">
                {cart.map((it) => (
                  <div key={it.id} className="border border-[#E5E7EB] rounded-lg p-2.5">
                    <div className="flex items-start gap-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-0.5">
                          <span className="text-[11px] text-[#6B7280]">Subtotal: {formatINR(it.price * it.qty)}</span>
                          <span className="text-[11px] font-bold text-[#0D9488]">{formatINR(it.price * it.qty)}</span>
                        </div>
            ...
            <div className="border-t border-[#E5E7EB] shrink-0">
              ...
              <div className="px-3 pb-3 space-y-2">
                <div className="grid grid-cols-3 gap-1.5">
                  <input value={mobile} onChange={(e) => setMobile(e.target.value)} placeholder="Mobile Number" className="h-10 rounded-lg border border-[#E5E7EB] px-2 text-[12px]" />
                  <input value={custName} onChange={(e) => setCustName(e.target.value)} placeholder="Customer Name" className="h-10 rounded-lg border border-[#E5E7EB] px-2 text-[12px]" />
                  <input value={tableNo} onChange={(e) => setTableNo(e.target.value)} placeholder="Table No" className="h-10 rounded-lg border border-[#E5E7EB] px-2 text-[12px]" />
                </div>
                <div>
...
function TopNav({ toggleSidebar, search, setSearch, tableNum }: { toggleSidebar: () => void; search: string; setSearch: (s: string) => void; tableNum: string }) {
  const items = [
  ...
  return (
    <div className="bg-white border-b border-[#E5E7EB] h-14 flex items-center gap-3 px-3 shrink-0">
      ...
      <div className="relative w-[200px] hidden md:block">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search menu items..." className="w-full h-9 pl-9 pr-3 rounded-full border border-[#E5E7EB] bg-white text-[13px]" />
      </div>
      <div className="flex items-center gap-2">
        <button onClick={() => { setSearch(""); window.scrollTo({ top: 0 }); }} className="h-9 px-3 rounded-lg bg-[#0D9488] hover:bg-[#0F766E] text-white text-[13px] font-semibold inline-flex items-center gap-1"><PlusIcon className="size-4" /> New Order</button>
        <span className="hidden lg:inline-flex h-9 px-3 rounded-lg bg-[#F59E0B] text-white text-[13px] font-semibold items-center">#{Math.floor(Math.random() * 9000) + 1000}</span>
        <button className="hidden lg:inline-flex h-9 px-3 rounded-lg border border-[#0D9488] text-[#0D9488] text-[13px] font-semibold items-center">SHORT CODE</button>
        ...
        <Link to="/tables" className="h-9 px-3 rounded-lg border border-[#DC2626] text-[#DC2626] text-[13px] font-semibold inline-flex items-center gap-1"><Armchair className="size-4" /> {tableNum ? `T${tableNum}` : "TABLES"}</Link>
      </div>
      <div className="ml-auto flex items-center">
        {items.map(({ to, icon: Icon, label, active }) => (
          <Link key={label} to={to}
            className={`flex flex-col items-center justify-center w-[52px] h-14 shrink-0 text-[10px] font-semibold relative ${active ? "text-[#0D9488]" : "text-[#6B7280] hover:text-[#111827]"}`}>
...
function DishGrid({ mode, dishes, cart, onAdd, onInc, onDec }: { mode: GridMode; dishes: Dish[]; cart: CartItem[]; onAdd: (d: Dish) => void; onInc: (id: string) => void; onDec: (id: string) => void }) {
    ...
    return (
      <div className="space-y-1.5 pb-3">
        {dishes.map((d, i) => {
          ...
          return (
            <div key={d.id} className={`bg-white rounded-lg border border-[#E5E7EB] px-3 py-2 flex items-center gap-2 ${!d.is_available ? "opacity-50" : ""}`}>
              ...
              <span className="text-[14px] font-semibold text-[#DC2626]">{formatINR(d.price)}</span>
              {!d.is_available ? <span className="text-[12px] text-[#9CA3AF]">Unavailable</span> :
                inCart ? (
                  <div className="inline-flex items-center border border-[#0D9488] rounded-md h-9">
                  ...
                  <button onClick={() => onAdd(d)} className="h-9 w-20 rounded-md border border-[#0D9488] text-[#0D9488] text-[13px] font-semibold hover:bg-[#0D9488] hover:text-white">+ Add</button>
                )}
            </div>
          );
        })}
  ...
  if (mode === "large") {
    return (
      <div className="grid gap-4 pb-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))" }}>
        {dishes.map((d, i) => {
          const inCart = cart.find((x) => x.id === d.id);
          return (
            <div key={d.id} className={`relative rounded-xl overflow-hidden border border-[#E5E7EB] bg-white h-[240px] ${!d.is_available ? "opacity-50" : ""}`}>
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3 flex items-end justify-between gap-2">
                ...
                {!d.is_available ? <span className="text-white text-[12px]">Unavailable</span> :
                  inCart ? (
                    <div className="inline-flex items-center bg-[#DC2626] rounded-lg h-10 text-white">
                    ...
                    <button onClick={() => onAdd(d)} className="h-10 px-4 rounded-lg bg-white text-[#DC2626] font-bold text-[13px]">+ ADD</button>
                  )}
              </div>
            </div>
          );
  ...
  // standard
  return (
    <div className="grid gap-3 pb-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))" }}>
      {dishes.map((d, i) => {
        const inCart = cart.find((x) => x.id === d.id);
        return (
          <div key={d.id} className={`rounded-xl border border-[#E5E7EB] bg-white overflow-hidden shadow-sm ${!d.is_available ? "opacity-50" : ""}`}>
            <div className="relative h-[160px] bg-[#0D9488]/10">
              ...
              <span className="absolute bottom-1.5 right-1.5 size-6 rounded-full bg-black/40 border border-white/60" />
            </div>
            <div className="px-3 py-2.5">
              ...
              <div className="text-[15px] font-semibold text-[#DC2626] mt-1">{formatINR(d.price)}</div>
              {!d.is_available ? (
                <div className="mt-2 h-10 rounded-md flex items-center justify-center text-[#9CA3AF] text-[13px]">Unavailable</div>
              ) : inCart ? (
                <div className="mt-2 inline-flex items-center justify-between w-full border border-[#0D9488] rounded-md h-10 text-[#0D9488]">
                  <button onClick={() => onDec(d.id)} className="size-10 inline-flex items-center justify-center"><Minus className="size-4" /></button>
                  <span className="font-bold text-[14px]">{inCart.qty}</span>
                  <button onClick={() => onInc(d.id)} className="size-10 inline-flex items-center justify-center"><Plus className="size-4" /></button>
                </div>
              ) : (
                <button onClick={() => onAdd(d)} className="mt-2 w-full h-10 rounded-md border border-[#0D9488] text-[#0D9488] text-[14px] font-semibold hover:bg-[#0D9488] hover:text-white transition-colors">+ ADD</button>
              )}
            </div>
          </div>
        );
  ...
  return (
    <div className="p-3 space-y-3">
      <div className="text-center">
        ...
        <span className="inline-block mt-1 bg-[#DCFCE7] text-[#16A34A] text-[11px] font-bold px-2 py-0.5 rounded-full">Confirmed</span>
        <div className="text-[14px] font-bold mt-2">Bill #{post.billNo} {isKot ? "sent to kitchen" : "completed"}</div>
        <div className="text-[12px] text-[#6B7280]">{isKot ? "Your order has been sent to the kitchen for preparation." : "Payment processed successfully. Thank you for your order!"}</div>
      </div>
...
Thank you for dining with us!`}
      ...
      <button onClick={onPrint} className={`w-full h-12 rounded-lg text-white font-bold text-[14px] inline-flex items-center justify-center gap-2 ${isKot ? "bg-[#F59E0B]" : "bg-[#2563EB]"}`}>
        <Printer className="size-4" /> {isKot ? "Print Kitchen Order (KOT)" : "Print Bill"}
      </button>
      <button onClick={onNew} className={`w-full h-12 rounded-lg text-white font-bold text-[14px] inline-flex items-center justify-center gap-1 ${isKot ? "bg-[#0D9488]" : "bg-[#16A34A]"}`}>
